...
public class ManajemenGudang {
    ...
    // Menu untuk pilih cara pencarian
    public void cariBarang() {
        System.out.println("=== Menu Pencarian ===");
        System.out.println("1. Cari berdasarkan ID/Nama");
        System.out.println("2. Cari berdasarkan minimal stok");
        ...
        switch (pilihan) {
            case "1" -> {
                ...
                System.out.println("\n=== Hasil ===");
                cariBarang(keyword); // versi String
            }
            case "2" -> {
                System.out.print("Masukkan minimal stok: ");
                try {
                    int minStok = Integer.parseInt(input.nextLine().trim());
                    System.out.println("\n=== Hasil ===");
                    cariBarang(minStok); // versi int
                } catch (NumberFormatException e) {
    ...
    // Overloading versi String
    public void cariBarang(String keyword) {
        boolean ditemukan = false;
        for (Barang b : daftarBarang) {
            if (b.getIdBarang().toLowerCase().contains(keyword.toLowerCase()) ||
                    b.getNamaBarang().toLowerCase().contains(keyword.toLowerCase())) {
                b.tampilkanInfo();
                ditemukan = true;
            }
        }
    ...
    // Overloading versi int
    public void cariBarang(int minStok) {
        boolean ditemukan = false;
        for (Barang b : daftarBarang) {
            if (b.getStok() >= minStok) {
                b.tampilkanInfo();
                ditemukan = true;
            }
        }
        if (!ditemukan) {
            System.out.println("Tidak ada barang dengan stok >= " + minStok);
        }
    }
